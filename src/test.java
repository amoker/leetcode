...
public class test {
    ...
    public List<Integer> spiralOrder(int[][] matrix) {
        ...
        for(int i = 0;i<rows*cols;i++){
        	res.add(matrix[r][c]);
        	seen[r][c] = true;
        	int tempr = r + rowstep[di];
        	int tempc = c + colstep[di];
        	if(tempc <cols && tempr < rows && tempc >= 0 && tempr >=0 && !seen[tempr][tempc]){
        		r = tempr;
        		c = tempc;
        	}
        	else{
    ...
    public void rotate(int[][] matrix) {
    	...
    	for(int start = 0;start<m/2;start++){
    		...
    		for(int i = start;i<top;i++) tmp.add(matrix[start][i]);
    		for(int i = start;i <top;i++) {
    			tmp.add(matrix[i][top]);
    			matrix[i][top] = tmp.poll();
    		}
    		for(int i = top;i> start;i--) {
    			tmp.add(matrix[top][i]);
    			matrix[top][i] = tmp.poll();
    		}
    		for(int i = top;i> start;i--) {
    			tmp.add(matrix[i][start]);
    			matrix[i][start] = tmp.poll();
    		}
    		for(int i = start;i<top-1;i++) matrix[start][i] = tmp.poll();
    	}
    }
    ...
    public void setZeroes(int[][] matrix) {
    	...
    	for(int[] row:matrix){
    		...
    		if(clear) Arrays.fill(row, 0);
    	}
    	for(int j = 0;j<col.length;j++) if(col[j]) for(int i = 0;i<matrix.length;i++) matrix[i][j] = 0;
    }
    ...
    public String minWindow(String s, String t) {
        int[] alph = new int[58];
        int[] alph2 = new int[58];
        for(char c:t.toCharArray()) alph[c-'A']++;      
        int j = 0;
        char[] chars = s.toCharArray();
        for(;j<s.length();j++){
        	if(minWindowHelper(alph, alph2, '!', chars[j]) == t.length()) break;
        }
        String res = "";
        if(count != t.length()) return res;
        int i = 0;
        boolean checknew = true;
        while(j<s.length()-1 || checknew){
        	if(checknew){
                while(minWindowHelper(alph, alph2, chars[i++], '!') == t.length()) ;
                i--;
                alph2[chars[i]-'A']++;
                count++;
        	...
        	else if(minWindowHelper(alph, alph2, chars[i++], chars[++j]) == t.length()) checknew = true;
        }
        return res;
    }
    ...
    public boolean exist(char[][] board, String word) {
    	for(int i = 0; i<board.length;i++){
    		for(int j = 0; j<board[0].length;j++){
    			if(existDFS(board, i, j, word)) return true;
    		}
    	}
    	return false;
    }
    ...
    public boolean existDFS(char[][]board, int i, int j, String word){
    	...
    	return existDFS(board, i+1, j, word.substring(1)) || existDFS(board, i, j+1, word.substring(1)) || existDFS(board, i-1, j, word.substring(1)) || existDFS(board, i, j-1, word.substring(1));
    }
    
    
    private int minWindowHelper(int[] alph, int[] alph2, char i, char c){
    	if(i != '!' && alph2[i-'A'] -- <= alph[i-'A']) count--;
    	if(c != '!' && alph2[c-'A'] ++< alph[c-'A']) count++;
    	return count;
    }
    ...
    public TreeNode MirrorHelper(TreeNode root){
        if(root == null) return null;
        root.right = MirrorHelper(root.left);
    ...
    public ArrayList<ArrayList<Integer>> FindContinuousSequence(int sum) {
        ...
        int[] dp = new int[(1+sum)/2 + 1];
        int j = 1;
        for(; j<dp.length; j++){
            dp[j] = dp[j-1] + j;
            if(dp[j]>=sum) break;
        }
        for(int i =0;j<dp.length;j++){
        	dp[j] = dp[j-1] + j;
            while(dp[j] - dp[i] >=sum){
                if(dp[j] - dp[i] == sum){
                    ArrayList<Integer> list = new ArrayList<>();
                    for(int k = i + 1; k <=j; k++) list.add(k);
    ...
    public String LeftRotateString(String str,int n) {
        ...
        StringBuilder sb1 = new StringBuilder(str.substring(0, n+1)).reverse();
        StringBuilder sb2 = new StringBuilder(str.substring(n+1)).reverse();
        return sb1.append(sb2).reverse().toString();
    }

	public static void main(String[] args) {
		test a = new test();
		System.out.println(a.LeftRotateString(null, 6));
	}
}